...
$conn = db_connect(); // Needed early for fetching
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Ensure the submitted ID matches the one from GET, for security/consistency
    ...
    if ($submitted_invoice_id !== $invoice_id) {
        ...
        // Potentially malicious attempt or form error
    }

    // Sanitize and validate main invoice details (similar to create_invoice.php)
    ...
    $invoice_number_posted = isset($_POST['invoice_number']) ? sanitize_string($_POST['invoice_number']) : $invoice_data['invoice_number']; // Keep original if not posted/editable
    ...
    for ($i = 0; $i < $item_count; $i++) {
        ...
        if (!empty($item_description) && $item_quantity > 0 && $item_unit_price >= 0) {
            ...
            $invoice_items_submitted[] = [
                'package_id' => $item_package_id ?: 'NULL',
    ...
    if (empty($errors)) {
        ...
        $sql_update_invoice = "UPDATE invoices SET
            client_id = $client_id,
            invoice_number = '$invoice_number_posted',
        ...
        if ($queries_ok) {
            ...
            // Repopulate form with submitted data if error, or re-fetch original if preferred
            // For now, the form will show original data on error if not POSTed again.
            // To show submitted data, we'd need to assign POST values back to $invoice_data and $invoice_items_data_existing
            $invoice_data['client_id'] = $_POST['client_id']; // Example of repopulating
            // ... and so on for all fields and items. This can be extensive.
            // For simplicity, this example doesn't fully repopulate items on error, they'd revert to original fetch.
        }
        mysqli_autocommit($conn, true); // Restore autocommit
    }
    // If errors, $invoice_data and $invoice_items_data_existing still hold the originally fetched data for form display.
    // To show user's attempted changes on error, you'd update these arrays with $_POST data.
    // For simplicity, this example will show the original data if an error occurs during the POST processing.
    // Let's try to reflect POSTed values if an error occurs:
    if(!empty($errors)){
        ...
        // Repopulating items is more complex. We'd need to rebuild $invoice_items_data_existing
        // from the $_POST['item_package_id'], etc. arrays.
        // For this iteration, items part of the form will use their values from the loop that processes POST data
        // or the JS will handle their current state on the client side if no full page reload.
        // Since PHP reloads, the items part of the form needs explicit repopulation.
        $temp_items_from_post = [];
        if(isset($_POST['item_package_id'])) {
            for ($i = 0; $i < count($_POST['item_package_id']); $i++) {
                 $temp_items_from_post[] = [
                    'package_id' => $_POST['item_package_id'][$i],
                    'item_description' => $_POST['item_description'][$i],
                    'quantity' => $_POST['item_quantity'][$i],
                    'unit_price' => $_POST['item_unit_price'][$i],
                 ];
            }
            // Overwrite $invoice_items_data_existing with submitted items if POST fails
            // This is a simplified way to handle it. A more robust method would ensure
            // the structure matches what the form expects.
            $invoice_items_data_existing = $temp_items_from_post;
        }
    }
...
<form action="edit_invoice.php?id=<?php echo $invoice_id; ?>" method="post" id="invoiceForm">
    ...
    <input type="hidden" name="invoice_number" value="<?php echo htmlspecialchars($invoice_data['invoice_number']); ?>"> <!-- Keep original number or make editable -->
    ...
    <fieldset>
        ...
        <table id="invoiceItemsTable">
            ...
            <tbody>
                <?php
                $item_rows_to_display = max(5, count($invoice_items_data_existing)); // Show at least 5 rows or more if data exists
                for ($i = 0; $i < $item_rows_to_display; $i++):
                    $current_item = $invoice_items_data_existing[$i] ?? null; // Get item if exists for this row
                    $selected_package_id = $current_item['package_id'] ?? '';
                    $item_description_val = $current_item['item_description'] ?? '';
                    $item_quantity_val = $current_item['quantity'] ?? '1';
                    $item_unit_price_val = isset($current_item['unit_price']) ? number_format($current_item['unit_price'], 2, '.', '') : '';
                ?>
                <tr>
                    <td>
                        <select name="item_package_id[]" class="package-select" data-row-id="<?php echo $i; ?>">
                            ...
                            <?php foreach ($packages as $package): ?>
                                <option value="<?php echo $package['id']; ?>"
                                        ...
                                        data-description="<?php echo htmlspecialchars($package['name']); ?>"
                                        <?php echo ($selected_package_id == $package['id']) ? 'selected' : ''; ?>>
    ...
    <fieldset>
        <legend>Financials</legend>
         <div class="form-group">
            ...
            <input type="text" id="sub_total_display" value="<?php echo number_format($invoice_data['sub_total'] ?? 0, 2, '.', ''); ?>" readonly>
        </div>
        <div class="form-group">
            <label for="tax_percentage">Tax (%):</label>
            <input type="text" name="tax_percentage" id="tax_percentage" value="<?php echo number_format($invoice_data['tax_percentage'] ?? 0, 2, '.', ''); ?>" pattern="^\d+(\.\d{1,2})?$">
        </div>
        <div class="form-group">
            <label for="tax_amount_display">Tax Amount:</label>
            <input type="text" id="tax_amount_display" value="<?php echo number_format($invoice_data['tax_amount'] ?? 0, 2, '.', ''); ?>" readonly>
        </div>
        <div class="form-group">
            <label for="discount_amount">Discount (Amount):</label>
            <input type="text" name="discount_amount" id="discount_amount" value="<?php echo number_format($invoice_data['discount_amount'] ?? 0, 2, '.', ''); ?>" pattern="^\d+(\.\d{1,2})?$">
        </div>
        <div class="form-group">
            <label for="grand_total_display">Grand Total:</label>
            <input type="text" id="grand_total_display" value="<?php echo number_format($invoice_data['grand_total'] ?? 0, 2, '.', ''); ?>" readonly>
        </div>
         <div class="form-group">
            <label for="amount_paid">Amount Paid:</label>
            <input type="text" name="amount_paid" id="amount_paid" value="<?php echo number_format($invoice_data['amount_paid'] ?? 0, 2, '.', ''); ?>" pattern="^\d+(\.\d{1,2})?$">
        </div>
    </fieldset>
...
<script>
// Re-use the same JavaScript from create_invoice.php for calculations
// Ensure it's robust enough for pre-filled data.
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateItemRow(rowId, selectedPackageElement) {
        const row = selectedPackageElement ? selectedPackageElement.closest('tr') : document.querySelector(`#invoiceItemsTable tbody tr:nth-child(${parseInt(rowId) + 1})`);
        if (!row) return;

        const descriptionInput = row.querySelector('.item-description');
        const unitPriceInput = row.querySelector('.item-unit-price');

        if (selectedPackageElement && selectedPackageElement.value !== "") {
            const price = selectedPackageElement.options[selectedPackageElement.selectedIndex].getAttribute('data-price');
            const description = selectedPackageElement.options[selectedPackageElement.selectedIndex].getAttribute('data-description');
    ...
    packageSelects.forEach(function(select) {
        ...
        // Initialize on load for pre-filled data
    ...
    if(amountPaidInput) amountPaidInput.addEventListener('input', calculateOverallTotals); // Amount paid doesn't change grand total, but might be used for balance due display if added

    // Initial calculation for all rows and overall totals on page load
    const rowCount = table.querySelectorAll('tbody tr').length;
    for(let i = 0; i < rowCount; i++){
        calculateRowTotal(i); // This will also trigger calculateOverallTotals
    }
    // One final call to ensure overall totals are correct after all rows processed
    calculateOverallTotals();
});
</script>
